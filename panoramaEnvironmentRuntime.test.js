...
test("F-E17b: 立方体名牌——与人偶名牌共存、顶面定位、不透明常显材质、自定义名", () => {
  const scene = new THREE.Scene();
  const gm = new THREE.Object3D(); gm.position.set(0, 0, 0);
  const gc = new THREE.Object3D(); gc.position.set(5, 1.5, -2); gc.scale.set(3, 3, 3);
  const bridge = {
    _mannequinMap: new Map([["m1", { group: gm }]]),
    _cubeMap: new Map([["k1", { group: gc }]]),
  };
  const sceneNode = { mannequins: [{ id: "m1" }], cubes: [{ id: "k1" }] };
  const ext = { cubes: { k1: { name: "残墙" } } };
  syncMannequinLabelsWith(bridge, ext, sceneNode, scene);
  const layer = bridge.__hyLabelLayer;
  assert.equal(layer.children.length, 2, "人偶+立方体名牌共存(各一)，不互相删");
  const cube = bridge.__hyLabelSprites.get("k1");
  assert.ok(cube, "立方体名牌已建");
  assert.equal(cube.text, "残墙", "立方体名牌用自定义名(directorExt.cubes[id].name)");
  // 顶面 y = position.y + 0.5·scale.y + LABEL_OFFSET(0.3) = 1.5 + 1.5 + 0.3 = 3.3
  assert.ok(Math.abs(cube.sprite.position.y - 3.3) < 1e-9, "立方体名牌在顶面上方 y=3.3");
  assert.ok(Math.abs(cube.sprite.position.x - 5) < 1e-9 && Math.abs(cube.sprite.position.z - (-2)) < 1e-9, "x/z 跟随立方体");
  // 材质:不透明常显（depthTest:false 永远置顶、alphaTest 丢透明边、transparent）
  const mat = cube.sprite.material;
  assert.equal(mat.depthTest, false, "depthTest:false → 名牌永远置顶,不被世界几何遮挡(回退回归)");
  assert.equal(mat.alphaTest, 0.5, "alphaTest 丢弃透明边距像素");
  assert.equal(mat.transparent, true);
  assert.equal(cube.sprite.renderOrder, 9999, "超高 renderOrder → 永远最后画,不被天空穹顶/地面遮挡");
});

test("F-E17c: 对象被隐藏(visibility) → 其头顶名牌随之移除(人偶+立方体一致)", () => {
  const scene = new THREE.Scene();
  const gm = new THREE.Object3D();
  const gc = new THREE.Object3D(); gc.scale.set(1, 1, 1);
  const bridge = {
    _mannequinMap: new Map([["m1", { group: gm }]]),
    _cubeMap: new Map([["k1", { group: gc }]]),
  };
  const sceneNode = { mannequins: [{ id: "m1" }], cubes: [{ id: "k1" }] };
  syncMannequinLabelsWith(bridge, {}, sceneNode, scene);
  assert.equal(bridge.__hyLabelLayer.children.length, 2, "都显示时两个名牌");
  // 隐藏立方体 k1 + 人偶 m1
  syncMannequinLabelsWith(bridge, { visibility: { cubeIds: ["k1"], mannequinIds: ["m1"] } }, sceneNode, scene);
  assert.equal(bridge.__hyLabelLayer.children.length, 0, "隐藏后名牌都被移除");
  assert.equal(bridge.__hyLabelSprites.has("k1"), false, "立方体名牌已回收");
  assert.equal(bridge.__hyLabelSprites.has("m1"), false, "人偶名牌已回收");
});
...
function makeFocalBridge({ clampMax = Infinity } = {}) {
  const b = { calls: [], _default: 35, _defaultSceneFocalLength: 35 };
  b.setDefaultSceneFocalLength = (v) => { b._default = Math.min(clampMax, Number(v)); b._defaultSceneFocalLength = b._default; b.calls.push(Number(v)); };
...
test("FOV-5: 桥默认焦距被外部回退后,同值也要重推（修 FOV“调了没用/卡死”）", () => {
  const b = makeFocalBridge();
  const node = { viewport: { sceneView: { focalLength: 24 } } };
  syncSceneViewFocalLengthWith(b, node);
  assert.deepEqual(b.calls, [24], "首推 24");
  // 模拟桥内部把默认焦距悄悄回退到 50（换视图等）—— 此时 store 仍是 24
  b._default = 50; b._defaultSceneFocalLength = 50;
  syncSceneViewFocalLengthWith(b, node); // 同 sceneView 焦距 24
  assert.deepEqual(b.calls, [24, 24], "桥回退后同值也重推（旧逻辑只比请求标记会误跳过 → FOV 卡在 50）");
  assert.equal(b._defaultSceneFocalLength, 24, "重推后桥默认焦距回到 24");
  syncSceneViewFocalLengthWith(b, node); // 桥未再回退 → 幂等跳过
  assert.deepEqual(b.calls, [24, 24], "桥值与上次应用一致 → 不再重复推（不自循环）");
});
