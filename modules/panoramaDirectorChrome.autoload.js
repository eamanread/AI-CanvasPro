...
import { getPanoramaSceneBridge, setPanoramaCameraHelpersVisible } from "./panoramaSceneNode/panoramaEnvironmentRuntime.js";
import { pickLocalImageAndCreatePanoramaBackground } from "./panoramaSceneBgLocalUpload.js";
// 需求③ 自动摆台:剧情/参考图 → 抽取 → 编译 → 清空重摆(全在 src/core 纯逻辑,已单测)
...
(function installDirectorChrome() {
  ...
  function renderTree(panel, shell, nodeId) {
    ...
    const cubeSelId = scene.selection?.selectedObjectType === "cube" ? scene.selection.selectedObjectId : null;
    const dcCamId = shell.dataset.dcCameraId || null;
    const sig = JSON.stringify({ tree, mannequinSelId, cubeSelId, dcCamId });
    ...
    const addRow = (item) => {
      ...
      const selIdForType = item.type === "camera" ? dcCamId : (item.type === "cube" ? cubeSelId : mannequinSelId);
      const selected = String(item.id) === String(selIdForType);
      ...
      if ((item.type === "mannequin" || item.type === "cube") && item.colorHex) icon.style.color = item.colorHex;
      row.appendChild(icon);
      const labelEl = el("span", "hy-dc-tree-label", item.label);
      row.appendChild(labelEl);
      // 人偶/立方体：双击标签 → 行内改名（落 directorExt.<mannequins|cubes>[id].name；空串清回派生标签）
      if (item.type === "mannequin" || item.type === "cube") {
        labelEl.addEventListener("dblclick", (e) => {
          stop(e);
          beginInlineRename(shell, nodeId, row, labelEl, item.id, item.type);
        });
      }
  ...
  function beginInlineRename(shell, nodeId, row, labelEl, objectId, kind) {
    ...
    let committed = false;
    const namePatch = kind === "cube" ? cubeNamePatch : mannequinNamePatch; // 立方体/人偶共用一套改名逻辑
    const commit = (save) => {
      if (committed) return;
      committed = true;
      if (save) writeDirectorExt(nodeId, namePatch(objectId, input.value));
  ...
  function clearSelectionIfLocked(shell, nodeId) {
    ...
    // 立方体同款：锁定当前选中的立方体 → 清选中（否则检查器还在，能继续改变换/缩放，锁定形同虚设）
    if (sel?.selectedObjectType === "cube" && sel.selectedObjectId
        && isObjectLocked("cube", sel.selectedObjectId, ext)) {
      setPanoramaSceneSelection({ nodeId, objectType: "cube", objectId: null, storeInstance: appStore });
    }
  ...
  function renderInspector(panel, shell, nodeId) {
    ...
    // 机位选中走导演侧 dcCameraId（原生 selection 不支持 camera）；原生选了人偶/立方体则优先它
    // （修：原门控只认 mannequin，选中 cube 时 insp 恒 null → 落到“3D场景”全局面板，cube 检查器整支死代码）
    const st = scene.selection?.selectedObjectType;
    const nativeSel = (st === "mannequin" || st === "cube") && scene.selection?.selectedObjectId;
    let dcCamId = shell?.dataset?.dcCameraId || "";
    if (nativeSel && dcCamId && shell) { delete shell.dataset.dcCameraId; dcCamId = ""; }
    let insp = null;
    if (nativeSel) insp = deriveInspector(scene, node?.directorExt); // 内部按 selectedObjectType 自分流 mannequin/cube
    else if (dcCamId) {
    ...
    } else if ((insp.kind === "mannequin" || insp.kind === "cube") && insp.editable) {
      // 人偶/立方体名称可编辑（落 directorExt.<mannequins|cubes>[id].name；空串清回派生标签）。同一套规则。
      const np = insp.kind === "cube" ? cubeNamePatch : mannequinNamePatch;
      const nameInput = el("input", "hy-dc-insp-name");
      nameInput.type = "text";
      nameInput.value = insp.label;
      nameInput.addEventListener("pointerdown", stop);
      nameInput.addEventListener("change", () => {
        writeDirectorExt(nodeId, np(insp.id, nameInput.value));
        refreshChrome(shell, nodeId, true);
      });
      nameRow.appendChild(nameInput);
    } else {
    ...
    if (insp.kind === "mannequin") {
    ...
    } else if (insp.kind === "cube") {
      // 立方体属性：位置/旋转/缩放 三轴横排 + 统一缩放（镜像人偶，无姿势/颜色 tab）。
      const writeXform = (key, value) => {
        const live = deriveInspector(sceneOf(getState()?.nodes?.[nodeId]));
        if (!live || live.kind !== "cube" || live.id !== insp.id) return;
        const patch = buildMannequinPosePatch(key, value, live.fields);
        if (patch) updatePanoramaSceneObjectTransform({ nodeId, objectType: "cube", objectId: insp.id, pose: patch, storeInstance: appStore });
      };
      xyzGroup(body, "位置", [["posX", "X"], ["posY", "Y"], ["posZ", "Z"]], "0.1", insp, writeXform);
      xyzGroup(body, "旋转", [["rotXDeg", "X"], ["rotYDeg", "Y"], ["rotZDeg", "Z"]], "1", insp, writeXform);
      xyzGroup(body, "缩放", [["scale", "X"], ["scale", "Y"], ["scale", "Z"]], "0.01", insp, writeXform);
      body.appendChild(sliderRow("统一缩放", insp.fields.scale, 0.2, 5, 0.01, (v) => writeXform("scale", v)));
      ...
      camXyz3("位置", (a) => insp.position[a], (a, v) => {
        applyCameraEdit(nodeId, insp.id, { cameras: { [insp.id]: { position: { [a]: v } } } });
      }, "0.1");

      // 点1:旋转(偏航/俯仰)——re-aim 相机(位置不动、无缩放)。改任一轴时用【实时】另一轴+位置+注视距离重算注视点。
      const aimNow = () => deriveCameraInspector(sceneOf(getNode(nodeId)), getNode(nodeId)?.directorExt, insp.id) || insp;
      const applyAim = (yaw, pitch) => {
        const live = aimNow();
        const patch = cameraAimFromYawPitchPatch(insp.id, yaw, pitch, live.position, live.lookDistance);
        if (patch) applyCameraEdit(nodeId, insp.id, patch);
      };
      body.appendChild(el("div", "hy-dc-insp-group", "旋转（朝向）"));
      body.appendChild(sliderRow("偏航", insp.yawDeg, -180, 180, 1, (v) => applyAim(v, aimNow().pitchDeg)));
      body.appendChild(sliderRow("俯仰", insp.pitchDeg, -89, 89, 1, (v) => applyAim(aimNow().yawDeg, v)));
      ...
      tgtSel.addEventListener("change", () => { applyCameraEdit(nodeId, insp.id, cameraLookAtTargetPatch(insp.id, tgtSel.value)); refreshChrome(shell, nodeId, true); });
  ...
  // 点6:截图时不要相机标记 → 隐藏 _cameraMap 标记 + 同步重渲一帧 → 抓图 → 恢复。需运行中的桥。
  function captureCleanDataUrl(shell, nodeId) {
    const bridge = getPanoramaSceneBridge(nodeId);
    let hidden = 0;
    if (bridge) {
      try {
        hidden = setPanoramaCameraHelpersVisible(bridge, false);
        if (hidden && typeof bridge.renderNow === "function") bridge.renderNow(); // 让"无相机标记"的帧进入绘制缓冲再抓
      } catch (_) { /* 桥不可用 → 退化为普通截图 */ }
    }
    const url = captureCameraDataUrl(shell);
    if (bridge && hidden) {
      try { setPanoramaCameraHelpersVisible(bridge, true); if (typeof bridge.renderNow === "function") bridge.renderNow(); } catch (_) {}
    }
    return url;
  }
  ...
  function captureWithAspectToCanvas(shell, nodeId) {
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
      const raw = captureCleanDataUrl(shell, nodeId);
  ...
  function renderCameraCaptureGallery(body, shell, nodeId, cameraId) {
    ...
    shotBtn.addEventListener("click", (e) => {
      stop(e);
      const url = captureCleanDataUrl(shell, nodeId);
  ...
  // 点5:按需渲染下,桥对焦距/位姿做渐变(lerp),渲染一停 lerp 就卡在半路 → FOV"调了没用/到不了目标值"。
  //   实测:持续 renderNow 约 10 帧焦距即收敛到正确视野。故提交相机视图后驱动若干帧把 lerp 跑到位;
  //   拖动滑杆时每次输入会 cancel 上一次、重启驱动(收敛 + 不堆叠 rAF)。
  let __hySettleRaf = 0;
  function settleSceneRender(nodeId) {
    const bridge = getPanoramaSceneBridge(nodeId);
    if (!bridge || typeof bridge.renderNow !== "function") return;
    if (__hySettleRaf) cancelAnimationFrame(__hySettleRaf);
    let n = 0;
    const tick = () => { try { bridge.renderNow(); } catch (_) { /* 渲染失败不拖累 */ } __hySettleRaf = (++n < 40) ? requestAnimationFrame(tick) : 0; };
    __hySettleRaf = requestAnimationFrame(tick);
  }
  function maybeApplyCameraView(nodeId, cameraId) {
    ...
    if (sv) {
      applyPanoramaSceneViewCommit({ nodeId, sceneView: sv, activeView: "default", activeCameraId: null, storeInstance: appStore });
      settleSceneRender(nodeId); // 点5:驱动渲染把焦距 lerp 跑到目标视野(否则 FOV 卡半路)
    }
  }
  ...
  function renderSceneGlobal(body, nodeId, g) {
    ...
    // 点8:白天/黑夜(固定两档,切换即改 3D 舞台灯光/色调;runtime applyEnvironmentMode 落地)
    const envRow = el("div", "hy-dc-insp-row hy-dc-envmode-row");
    envRow.appendChild(el("label", "hy-dc-insp-label", "时间"));
    const envSeg = el("div", "hy-dc-envmode-seg");
    for (const [mode, label] of [["day", "白天"], ["night", "黑夜"]]) {
      const b = el("button", "hy-dc-envmode-btn" + (g.environmentMode === mode ? " is-active" : ""), label);
      b.type = "button";
      b.addEventListener("pointerdown", stop);
      b.addEventListener("click", (e) => {
        stop(e);
        setPanoramaSceneEnvironmentMode({ nodeId, environmentMode: mode, storeInstance: appStore }); // 记录档位(钮高亮+持久化)
        // 点8(改正):白天/黑夜改的是【头顶天空颜色】,不是给人物/背景打光。写 environmentSkyColor → 经天空色路径落 scene.background(见点7)。
        writeDirectorExt(nodeId, skyColorPatch(mode === "night" ? "#0c1228" : "#bcdcf2"));
        const br = getPanoramaSceneBridge(nodeId);
        if (br && typeof br.renderNow === "function") { br.renderNow(); br.renderNow(); }
        envSeg.querySelectorAll(".hy-dc-envmode-btn").forEach((x) => x.classList.remove("is-active"));
        b.classList.add("is-active");
      });
      envSeg.appendChild(b);
    }
    envRow.appendChild(envSeg);
  ...
  // 点1:把检查器编辑后的【机位位置+朝向】同步进 sceneNode.cameras[id]（position + quaternion），
  // 使桥 _syncCameras 重摆+重转「相机标记」——标记的位姿只认 sceneNode（不读 directorExt），故仅写 directorExt
  // 时视图(orbit)变了而 3D 场景里的相机图标【纹丝不动】(用户实测的 bug)。这里用与 buildCameraSceneView 同一套
  // (位置, 注视点) 解析,保证 标记 ≡ 视图 ≡ 检查器 三者一致。朝向用 lookAtQuaternion(桥相机前向 -Z)。
  function syncCameraMarker(nodeId, cameraId) {
    const node = getNode(nodeId);
    const scene = sceneOf(node);
    if (!scene || !Array.isArray(scene.cameras)) return;
    const cam = scene.cameras.find((c) => String(c.id) === String(cameraId));
    if (!cam) return;
    const ext = (node?.directorExt?.cameras && node.directorExt.cameras[String(cameraId)]) || {};
    const position = ext.position && Number.isFinite(ext.position.x) ? ext.position : cam.position;
    const lookAtPoint = resolveCameraLookAtPoint(ext, scene.mannequins);
    const quaternion = lookAtQuaternion(position, lookAtPoint);
    if (!position || !Number.isFinite(position.x) || !quaternion) return;
    const cameras = scene.cameras.map((c) => (String(c.id) === String(cameraId)
      ? { ...c, position: { x: position.x, y: position.y, z: position.z }, quaternion }
      : c));
    appStore.updateNodeData(nodeId, { sceneNode: { ...scene, cameras } });
    // ⚠️ 写 sceneNode 后桥【不会】自动重摆「相机标记」:渲染钩子(syncPanoramaEnvironment)只同步天空/网格,
    //    且常态下无人移动机位 → 相机重摆路径从未被触发。必须手动调桥的 _syncCameras(把标记摆到 camera.position
    //    + 转到 quaternion;真机实测 marker.position≡camera.position、quaternion 直接 set 上)。
    //    必须传【sceneNode】——传 node(bridge.sync(node)) 会因取不到 node.cameras 把所有相机标记清空(真机踩过)。
    const bridge = getPanoramaSceneBridge(nodeId);
    const updated = getNode(nodeId)?.sceneNode;
    if (bridge && updated && typeof bridge._syncCameras === "function") {
      try { bridge._syncCameras(updated); } catch (_) { /* 桥内部异常不拖累编辑 */ }
    }
  }
  // 机位 位置/朝向 编辑统一入口:写 directorExt(检查器显示 + orbit 视图取数) → 同步标记位姿(sceneNode) → 套用视图 →
  // settle 驱动渲染(任何视角都落屏:导演视角下 maybeApplyCameraView 会提前返回不驱动,故这里兜底驱动一次)。
  function applyCameraEdit(nodeId, cameraId, directorPatch) {
    if (directorPatch) writeDirectorExt(nodeId, directorPatch);
    syncCameraMarker(nodeId, cameraId);
    maybeApplyCameraView(nodeId, cameraId);
    settleSceneRender(nodeId);
  }

  // ====== 点1:在 3D 场景里【拖动相机标记】移动机位(像拖人偶) ======
  // 经 workflow 可行性勘察+对抗评审定为最低风险路线(不碰混淆桥的 selection/gizmo/_pickRoots)。真机已逐项验证:
  //  · 相机标记是 LineSegments 线框且不在 bridge._pickRoots(_rebuildPickRoots 每次 _syncMannequins/_syncCubes 清空且永不收相机)
  //    → 不用 bridge.pick();自持 raycaster(bridge._resolvePointerRay 取射线)对 _cameraMap 的 group 直接 intersect,调 Line.threshold 容差命中线框。
  //  · 标记父级是 Scene(单位矩阵)→ 世界坐标 == sceneNode.cameras[i].position 空间,bridge.intersectGround 的世界点可直接当机位坐标。
  //  · 捕获相 pointerdown 抢在原生交互层(viewportEl 冒泡 pointerdown + setPointerCapture)与 orbit 之前;命中相机才 stopPropagation,
  //    否则【完全放行】→ orbit/人偶拖拽零影响。move/up 绑 document 捕获相收全程。
  //  · 拖拽中只 set group.position + renderNow 实时跟手(不喂 _syncCameras——它会按 active/sceneView 等价 重算 group.visible,
  //    可能把标记拖一半隐藏);pointerup 才走 applyCameraEdit 一次落库(写 directorExt + sceneNode.cameras + _syncCameras)。
  //  · 仅导演/orbit 视角(标记可见)启用;机位视角(标记隐藏)不接管。只移动(禁缩放天然成立);朝向用标记自身四元数推前向→平移注视点保持不变;旋转走检查器 偏航/俯仰。
  const CAM_PICK_THRESHOLD = 0.09; // 线框拾取容差(世界单位;标记约 0.26m,留抓取余量)
  function cameraForwardFromQuat(q) {
    // 相机前向 = q·(0,0,-1)(桥约定 -Z 朝前)
    return { x: -2 * (q.x * q.z + q.w * q.y), y: -2 * (q.y * q.z - q.w * q.x), z: -(1 - 2 * (q.x * q.x + q.y * q.y)) };
  }
  function pickCameraMarkerAt(bridge, clientX, clientY) {
    const map = bridge && bridge._cameraMap;
    if (!map || !map.size || typeof bridge._resolvePointerRay !== "function") return null;
    const entries = [...map.entries()];
    const groups = entries.map(([, e]) => e && (e.group || e.object3d)).filter(Boolean);
    if (!groups.length) return null;
    const ray = bridge._resolvePointerRay(clientX, clientY);
    if (!ray || typeof ray.intersectObjects !== "function") return null;
    ray.params = ray.params || {};
    ray.params.Line = Object.assign({}, ray.params.Line, { threshold: CAM_PICK_THRESHOLD });
    ray.params.Points = Object.assign({}, ray.params.Points, { threshold: CAM_PICK_THRESHOLD });
    let hits;
    try { hits = ray.intersectObjects(groups, true); } catch (_) { return null; } // raycaster 默认跳过 visible:false → 命中即可见
    for (const h of hits) {
      let o = h.object;
      while (o) {
        const found = entries.find(([, e]) => (e && (e.group || e.object3d)) === o);
        if (found) {
          const grp = found[1].group || found[1].object3d;
          return grp && grp.visible ? { camId: String(found[0]), group: grp } : null;
        }
        o = o.parent;
      }
    }
  ...
  let __hyCamDragActive = false;
  function beginCameraMarkerDrag(shell, nodeId, bridge, camId, group, startGround) {
    const node = getNode(nodeId);
    const insp = deriveCameraInspector(sceneOf(node), node && node.directorExt, camId);
    if (!insp || !insp.position) return;
    const startPos = { x: Number(insp.position.x) || 0, y: Number(insp.position.y) || 0, z: Number(insp.position.z) || 0 };
    const dist = insp.lookDistance > 0.01 ? insp.lookDistance : 5;
    const fwd = cameraForwardFromQuat(group.quaternion); // 用标记实际朝向 → 拖动时平移注视点保持朝向不变(避免首拖朝向跳变)
    const planeY = startPos.y;
    let last = startPos;
    let moved = false;
    __hyCamDragActive = true;
    shell.dataset.dcCameraId = camId; // 复用导演侧机位选中态(检查器随即显示该机位)
    refreshChrome(shell, nodeId, true);
    const live = (p) => { try { group.position.set(p.x, p.y, p.z); if (typeof bridge.renderNow === "function") bridge.renderNow(); } catch (_) {} };
    const onMove = (e) => {
      const g = bridge.intersectGround(e.clientX, e.clientY, planeY);
      if (!g) return; // 近平视/平面平行 → 跳过该帧(不让 null 当原点)
      const dx = g.x - startGround.x, dz = g.z - startGround.z;
      if (!Number.isFinite(dx) || !Number.isFinite(dz) || Math.hypot(dx, dz) > 1000) return; // 防爆破(近平视投影发散)
      if (Math.hypot(dx, dz) > 1e-4) moved = true;
      last = { x: startPos.x + dx, y: startPos.y, z: startPos.z + dz };
      live(last);
    ...
    const cleanup = () => {
      ...
      __hyCamDragActive = false;
    };
    const onUp = () => {
      cleanup();
      if (!moved) return; // 纯点击未拖动 → 只选中,不落库
      const np = last;
      const nl = { x: np.x + fwd.x * dist, y: np.y + fwd.y * dist, z: np.z + fwd.z * dist }; // 平移注视点=保持朝向
      applyCameraEdit(nodeId, camId, { cameras: { [camId]: { position: np, lookAtPoint: nl, lookAtTarget: "manual" } } });
      refreshChrome(shell, nodeId, true);
    };
    const onCancel = () => { cleanup(); live(startPos); }; // 取消 → 标记回滚到起点
    document.addEventListener("pointermove", onMove, true);
    document.addEventListener("pointerup", onUp, true);
    document.addEventListener("pointercancel", onCancel, true);
  }
  function onCameraMarkerPointerDown(ev) {
    if (__hyCamDragActive || ev.button !== 0 || !ev.target || typeof ev.target.closest !== "function") return;
    const shell = ev.target.closest(SHELL_SELECTOR);
    if (!shell || !shell.querySelector(`.${CHROME_CLASS}.is-visible`)) return; // 只在导演台激活的 shell
    if (getViewMode(shell) === "camera") return; // 机位视角标记隐藏,不拖
    const nodeId = resolveShellNodeId(shell);
    if (!nodeId) return;
    const bridge = getPanoramaSceneBridge(nodeId);
    if (!bridge) return;
    const hit = pickCameraMarkerAt(bridge, ev.clientX, ev.clientY);
    if (!hit) return; // 没点到相机标记 → 完全放行(orbit/人偶照常)
    const planeY = (hit.group.position && hit.group.position.y) || 0;
    const startGround = bridge.intersectGround(ev.clientX, ev.clientY, planeY);
    if (!startGround) return; // 退化 → 放行
    // 接管:抢在原生 viewportEl 冒泡 pointerdown(setPointerCapture)+ orbit 之前
    ev.stopPropagation();
    if (typeof ev.stopImmediatePropagation === "function") ev.stopImmediatePropagation();
    try { ev.preventDefault(); } catch (_) {}
    beginCameraMarkerDrag(shell, nodeId, bridge, hit.camId, hit.group, startGround);
  }
  function installCameraMarkerDragOnce() {
    if (window.__hyCamMarkerDragInstalled) return;
    window.__hyCamMarkerDragInstalled = true;
    document.addEventListener("pointerdown", onCameraMarkerPointerDown, true); // 捕获相,先于原生交互层
  }
  ...
  function buildCharacterPopup(shell, nodeId) {
    ...
    const items = [
      ...
      ["立方体", () => addPanoramaSceneCube({ nodeId, storeInstance: appStore })],
    ];
  ...
  // ---- 外壳装配与刷新 ----
  // ===== 需求③:底部"剧情/参考图 → 自动摆台"输入框(文本节点模型;无 @//;无图像生成项;上传+连边参考)=====
  function collectScriptRefImageUrls(nodeId) {
    const out = [];
    const si = getNode(nodeId)?.directorExt?.scriptInput || {};
    if (Array.isArray(si.refImages)) for (const u of si.refImages) if (u) out.push(String(u));
    try {
      const st = getState();
      const edges = st?.edges ? (Array.isArray(st.edges) ? st.edges : Object.values(st.edges)) : [];
      for (const e of edges) {
        if (String(e?.targetId ?? e?.target ?? "") !== String(nodeId)) continue;
        const src = st.nodes?.[String(e?.sourceId ?? e?.source ?? "")];
        const url = src?.src || src?.imageUrl || src?.localPath;
        if (url && /image|png|jpe?g|webp/i.test(String(src?.type || "") + " " + String(url))) out.push(String(url));
      }
    } catch (_) { /* 无边/取不到忽略 */ }
    return out;
  }
  function collectScriptRefTexts(nodeId) {
    // 连进来的「文本节点」(source-text/ai-text)内容 → 作为剧本一并喂给抽取模型。
    const out = [];
    try {
      const st = getState();
      const edges = st?.edges ? (Array.isArray(st.edges) ? st.edges : Object.values(st.edges)) : [];
      for (const e of edges) {
        if (String(e?.targetId ?? e?.target ?? "") !== String(nodeId)) continue;
        const src = st.nodes?.[String(e?.sourceId ?? e?.source ?? "")];
        if (!src || !/^(source-text|ai-text)$/.test(String(src.type || ""))) continue;
        const t = String(src.outputText || src.content || src.text || src.prompt || "").trim();
        if (t) out.push(t);
      }
    } catch (_) { /* 无边/取不到忽略 */ }
    return out;
  }
  function renderScriptRefs(refs, nodeId) {
    // 参考来自「连线」(图片/文本节点连到本节点),不再走上传;断线即自动减少。
    // 用生成节点同款缩略图规范:.ref-thumb-container > .ref-thumb-wrap > img.ref-thumb-media(44×44 圆角)。
    const imgs = collectScriptRefImageUrls(nodeId);
    const txts = collectScriptRefTexts(nodeId);
    const sig = JSON.stringify([imgs, txts]);
    if (refs.dataset.refsig === sig) return; // 参考集未变 → 不重建,避免 350ms 轮询每轮拆建 <img> 导致闪烁/重解码/丢 hover
    refs.dataset.refsig = sig;
    refs.replaceChildren();
    if (!imgs.length && !txts.length) return;
    const cont = el("div", "ref-thumb-container");
    for (const url of imgs) {
      const wrap = el("div", "ref-thumb-wrap");
      const img = document.createElement("img");
      img.className = "ref-thumb-media"; img.src = url; img.alt = "参考图"; img.draggable = false;
      wrap.appendChild(img);
      cont.appendChild(wrap);
    }
    for (const t of txts) { // 文本节点连入:无缩略图,给同尺寸「文」chip(hover 显内容)
      const wrap = el("div", "ref-thumb-wrap");
      const chip = el("div", "ref-thumb-media hy-dc-script-textref", "文");
      chip.title = String(t).slice(0, 120);
      wrap.appendChild(chip);
      cont.appendChild(wrap);
    }
    refs.appendChild(cont);
  }
  // (已删除 pickScriptRefImage 文件上传逻辑——参考图改走「连线」)
  // 执行:剧情/图 → 抽取(所选文本模型) → 编译 → 清空重摆。全程复用已单测纯逻辑。
  function findOpenShellForNode(nodeId) {
    for (const s of document.querySelectorAll(SHELL_SELECTOR)) {
      if (String(resolveShellNodeId(s)) === String(nodeId)) return s;
    }
    return null;
  }
  async function runScriptAutoStage(nodeId, statusEl, genBtn) {
    const node = getNode(nodeId); const scene = sceneOf(node); if (!scene) return;
    const si = node?.directorExt?.scriptInput || {};
    // 剧本 = 连进来的文本节点内容(按连边)+ 输入框文字,一并喂给抽取模型。
    const wiredTexts = collectScriptRefTexts(nodeId);
    const text = [...wiredTexts, String(si.prompt || "").trim()].filter(Boolean).join("\n\n").trim();
    const refUrls = collectScriptRefImageUrls(nodeId);
    const setStatus = (s) => { if (statusEl) statusEl.textContent = s; };
    if (!text && !refUrls.length) { setStatus("请输入剧本或加参考图"); return; }
    let models = []; try { models = getSelectableTextModels() || []; } catch (_) { models = []; }
    const model = models.find((m) => m.id === si.selectedModelId) || models[0];
    if (!model || !hasCompleteModelConfig(model)) { setStatus("所选文本模型未配置(去设置→文本节点模型)"); return; }
    const hasImage = refUrls.length > 0;
    const prompt = buildExtractionSystemPrompt({ hasImage }) + "\n\n" + buildExtractionUserPrompt(text, { hasImage });
    const payload = buildRegistryTextPayload({ prompt, inputUrls: refUrls }, model);
    if (genBtn) genBtn.disabled = true; setStatus("生成中…");
    let resp;
    try { resp = await generateTextWithRegistryModel(payload); }
    catch (e) { setStatus("调用失败:" + (e?.message || e)); if (genBtn) genBtn.disabled = false; return; }
    const blocking = parseBlockingJson(resp);
    if (!blocking) { setStatus("解析失败:模型未返回有效 JSON"); if (genBtn) genBtn.disabled = false; return; }
    const plan = buildRestagePlan(compileBlockingToScene(blocking), { idFor: (k, i) => `${k}-${Date.now()}-${i}` });
    const liveNode = getNode(nodeId); const liveScene = sceneOf(liveNode) || scene;
    appStore.updateNodeData(nodeId, {
      sceneNode: { ...liveScene, mannequins: plan.mannequins, cubes: plan.cubes, cameras: plan.cameras }, // 清空重摆=整组替换
      directorExt: { ...(liveNode?.directorExt || {}), ...plan.directorExt, scriptInput: liveNode?.directorExt?.scriptInput || si }, // 用最新 scriptInput,勿用 await 前的快照覆盖生成期间的编辑
    });
    setStatus(`已摆台:${plan.mannequins.length} 人 / ${plan.cubes.length} 道具 / ${plan.cameras.length} 机位`);
    if (genBtn) genBtn.disabled = false;
    const sh = findOpenShellForNode(nodeId);
    if (sh) refreshChrome(sh, nodeId, true); // 编辑器开着才重渲;没开则下次打开按新数据渲
  }
  // 模型选择器(文本节点模型),用设计规范的 .img-model-wrap/.img-model-menu 文字钮 + 弹层(由 bundle/S4.11 上皮肤)。
  const CARET_SVG = '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="node-menu-caret"><polyline points="6 9 12 15 18 9"></polyline></svg>';
  const ARROW_UP_SVG = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg>';
  function buildScriptModelSelector(nodeId) {
    const wrap = el("div", "img-model-wrap");
    let models = []; try { models = getSelectableTextModels() || []; } catch (_) { models = []; }
    const si = getNode(nodeId)?.directorExt?.scriptInput || {};
    const cur = models.find((m) => m.id === si.selectedModelId) || models[0] || null;
    const trigger = el("button", "img-pill-btn img-model-btn-trigger"); trigger.type = "button";
    const label = el("span", "img-model-label", cur ? cur.modelName : "无文本模型");
    trigger.appendChild(label);
    const caret = el("span", "node-menu-caret-wrap"); caret.innerHTML = CARET_SVG; trigger.appendChild(caret);
    const menu = el("div", "floating-menu img-model-menu node-model-menu");
    if (!models.length) { const it = el("div", "floating-menu-item is-disabled", "无文本模型(去设置→文本节点模型)"); menu.appendChild(it); }
    for (const m of models) {
      const it = el("div", "floating-menu-item" + (cur && m.id === cur.id ? " active" : ""), m.modelName + (hasCompleteModelConfig(m) ? "" : "·未配置"));
      it.addEventListener("pointerdown", stop);
      it.addEventListener("click", (e) => { stop(e); writeDirectorExt(nodeId, { scriptInput: { selectedModelId: m.id } }); label.textContent = m.modelName; menu.classList.remove("show"); menu.querySelectorAll(".floating-menu-item").forEach((x) => x.classList.remove("active")); it.classList.add("active"); });
      menu.appendChild(it);
    }
    trigger.addEventListener("pointerdown", stop);
    trigger.addEventListener("click", (e) => { stop(e); menu.classList.toggle("show"); });
    wrap.appendChild(trigger); wrap.appendChild(menu);
    return wrap;
  }
  // 走设计规范:直接复用 App 原生「停靠态生成面板」类——.text-prompt-panel + .viewport-fixed-prompt + .is-visible。
  //   · viewport-fixed-prompt 提供 position:fixed;bottom:10px;left:50%;width:min(616px,..);z-index 等停靠布局(不随画布缩放);
  //   · is-visible 提供 opacity:1;pointer-events:auto(基类默认 opacity:0 隐藏)。
  // 仅靠基类 .text-prompt-panel(position:absolute;opacity:0;scale(--zoom-inv))会塌成 0×0 且透明且被缩放——血泪教训,勿回退。
  // 页脚 .prompt-panel-footer(模型文字钮 + 白圆↑提交钮)+ .node-ref-bar 参考图,皮肤由 bundle/theme-upgrade.css 提供。
  function buildScriptDockContent(nodeId) {
    const panel = el("div", "text-prompt-panel viewport-fixed-prompt is-visible hy-dc-scriptpanel");
    panel.addEventListener("pointerdown", stop);
    const si = getNode(nodeId)?.directorExt?.scriptInput || {};
    const refs = el("div", "node-ref-bar hy-dc-script-refs");
    panel.appendChild(refs);
    const wrapIn = el("div", "prompt-input-wrapper");
    const ta = el("textarea", "prompt-textarea custom-textarea");
    ta.placeholder = "输入剧本 → 自动摆第一场站位(Enter 生成,Shift+Enter 换行;连图片/文本节点作参考)";
    ta.value = typeof si.prompt === "string" ? si.prompt : "";
    ta.rows = 3;
    ta.addEventListener("pointerdown", stop);
    ta.addEventListener("input", () => writeDirectorExt(nodeId, { scriptInput: { prompt: ta.value } }));
    // Enter 生成 / Shift+Enter 换行(对齐 gen 节点交互;无 @// 触发)
    ta.addEventListener("keydown", (e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); stop(e); runScriptAutoStage(nodeId, status, gen); } });
    wrapIn.appendChild(ta);
    panel.appendChild(wrapIn);
    const footer = el("div", "prompt-panel-footer text-prompt-actions");
    footer.appendChild(buildScriptModelSelector(nodeId));
    // 参考图不走上传按钮——改用「连线」:把图片/文本节点连到本导演台节点即作参考(见 isValidConnection + collectScriptRefImageUrls)。
    const status = el("span", "hy-dc-script-status", "");
    footer.appendChild(status);
    const actions = el("div", "prompt-actions");
    const gen = el("button", "prompt-submit img-gen-btn"); gen.type = "button"; gen.title = "生成站位"; gen.innerHTML = ARROW_UP_SVG;
    gen.addEventListener("pointerdown", stop);
    gen.addEventListener("click", (e) => { stop(e); runScriptAutoStage(nodeId, status, gen); });
    actions.appendChild(gen);
    footer.appendChild(actions);
    panel.appendChild(footer);
    renderScriptRefs(refs, nodeId);
    return panel;
  }
  // 页面底部 dock:选中【恰好一个 3D导演台节点】时浮出输入框(独立于 3D 视图,挂 document.body);否则收起。
  let scriptDockEl = null;
  let scriptDockNodeId = null;
  function ensureScriptDock() {
    if (scriptDockEl && document.body.contains(scriptDockEl)) return scriptDockEl;
    scriptDockEl = el("div", "hy-dc-scriptdock");
    document.body.appendChild(scriptDockEl);
    return scriptDockEl;
  }
  function resolveDockNodeId() {
    const st = getState();
    // ① 画布单选导演台节点(对齐文本/图片生成节点:点选即在页面底部浮出输入框)
    const sel = (st && Array.isArray(st.selectedNodeIds)) ? st.selectedNodeIds : [];
    if (sel.length === 1) {
      const n = st.nodes?.[sel[0]];
      if (n && isPanoramaSceneNodeType(n.type)) return String(sel[0]);
    }
    // 仅「点选导演台节点」才浮出(对齐文本/图片生成节点)。曾加过"shell 已挂载即显"的兜底②,
    // 但 .panorama-scene-shell 在画布上常驻挂载 → dock 变常驻(用户报错:必须点击时才显示)。故只认单选,删除兜底。
    return null;
  }
  function syncScriptDock() {
    const targetId = resolveDockNodeId();
    // 点2:仅全屏导演台时隐藏 dock(内联编辑/画布选中保留显示)。全屏面板整屏覆盖,底部输入框应让位。
    const inFullscreen = !!document.querySelector(FULLSCREEN_SELECTOR);
    if (!targetId || inFullscreen) {
      if (scriptDockEl) { scriptDockEl.style.display = "none"; scriptDockNodeId = null; }
      return;
    }
    const dock = ensureScriptDock();
    if (scriptDockNodeId !== targetId) { // 仅换节点(或首次)才重建,避免吹掉正在输入的文本
      scriptDockNodeId = targetId;
      dock.replaceChildren(buildScriptDockContent(targetId));
    } else {
      // 同节点:轻量刷新参考图条(用户可能刚连/断了图片节点的边)
      const refsEl = dock.querySelector(".hy-dc-script-refs");
      if (refsEl) renderScriptRefs(refsEl, targetId);
    }
  ...
  function ensureChrome(shell, nodeId) {
    ...
    if (!chrome) {
      ...
      // 需求③ 输入框不在 3D 覆盖层里——改为页面底部独立 dock(点节点浮出),见 ensureScriptDock/syncScriptDock。
  ...
  function syncOnce() {
    try { syncScriptDock(); } catch (_) { /* dock 独立于编辑器,出错不拖累 chrome */ }
  ...
  function activeDirectorContext() {
    // 多个导演台节点可同时挂载 shell + 可见 chrome(如上一个编辑过的节点 shell 没卸载)。必须选「真正活动的那个」,
    // 否则恒取 DOM 第一个(可能是另一个空节点)→ 方向键(点4)/FOV(点5)作用在错节点 → 用户在自己编辑的节点里“完全没反应”。
    // 优先级:① 全屏 shell(用户正全屏编辑)→ ② 含焦点的 shell → ③ 第一个有可见 chrome 的。
    const shells = [...document.querySelectorAll(SHELL_SELECTOR)].filter(
      (s) => s.querySelector(`.${CHROME_CLASS}.is-visible`),
    );
    if (!shells.length) return null;
    const shell =
      shells.find((s) => s.closest(FULLSCREEN_SELECTOR))
      || shells.find((s) => document.activeElement && s.contains(document.activeElement))
      || shells[0];
    const nodeId = resolveShellNodeId(shell);
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (selMan) {
      ...
      // 未选人偶 + 机位视角 → 画面平移(truck/pedestal):相机位置与注视点【同向】偏移 → 朝向不变,整幅画面左右/上下平移。
      // 偏移沿相机【屏幕右/上向量】(由前向量 × 世界上向量推),故倾斜机位也按"画面"方向平移,而非世界轴(原先 ↑↓ 是世界Z=推拉,错)。
      ...
      const look = resolveCameraLookAtPoint(ext, scene?.mannequins) || { x: Number(pos.x) || 0, y: Number(pos.y) || 0, z: (Number(pos.z) || 0) - 1 };
      let fx = (Number(look.x) || 0) - (Number(pos.x) || 0), fy = (Number(look.y) || 0) - (Number(pos.y) || 0), fz = (Number(look.z) || 0) - (Number(pos.z) || 0);
      const flen = Math.hypot(fx, fy, fz) || 1; fx /= flen; fy /= flen; fz /= flen;
      let rx = -fz, rz = fx; const rlen = Math.hypot(rx, rz) || 1; rx /= rlen; rz /= rlen; // right = norm(cross(fwd, 上(0,1,0)))
      const ux = -rz * fy, uy = rz * fx - rx * fz, uz = rx * fy; // up = cross(right, fwd)（right.y=0）
      const h = d[0], v = -d[1]; // 右=+右向量;ArrowUp(d[1]=-1)=+上向量
      const ox = (rx * h + ux * v) * ARROW_STEP, oy = (uy * v) * ARROW_STEP, oz = (rz * h + uz * v) * ARROW_STEP;
      const np = { x: (Number(pos.x) || 0) + ox, y: (Number(pos.y) || 0) + oy, z: (Number(pos.z) || 0) + oz };
      const nl = { x: (Number(look.x) || 0) + ox, y: (Number(look.y) || 0) + oy, z: (Number(look.z) || 0) + oz };
      writeDirectorExt(nodeId, { cameras: { [camId]: { position: np, lookAtPoint: nl, lookAtTarget: "manual" } } });
  ...
  // 点3:旧工具栏偶现根因——shell 挂载(原生工具栏默认可见)到 350ms 轮询装 chrome+隐原生 之间最长 350ms 空窗;
  //   期间 CSS `.panorama-scene-shell:has(.hy-director-chrome.is-visible) .panorama-scene-fixed-toolbar{display:none}` 因 chrome 还没 is-visible 不匹配 → 原生工具栏闪一下。
  //   修:监听 DOM,shell 一出现就立刻 syncOnce(装 chrome+加 is-visible+隐原生),把空窗压到一帧内(稳态那条 CSS 已验证够压)。
  const __hyShellMountObserver = new MutationObserver((muts) => {
    for (const m of muts) {
      for (const node of m.addedNodes) {
        if (node && node.nodeType === 1 && (node.matches?.(SHELL_SELECTOR) || node.querySelector?.(SHELL_SELECTOR))) {
          try { syncOnce(); } catch (_) { /* 装载失败不拖累轮询 */ }
          return;
        }
      }
    }
  });
  try { __hyShellMountObserver.observe(document.body, { childList: true, subtree: true }); } catch (_) {}
  installCameraMarkerDragOnce(); // 点1:相机标记 3D 拖动(捕获相 document pointerdown,命中相机才接管)
