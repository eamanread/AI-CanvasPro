...
export function applyObjectVisibilityWith(bridge, ext, sceneNode) {
  ...
  // visibility 用 mannequinIds 给人偶；方块复用 mannequinIds 命中也无害（cubeIds 暂不暴露 UI）
...
export function syncMannequinLabelsWith(bridge, ext, sceneNode, scene) {
  ...
  map.forEach((visual, id) => {
    ...
    seen.add(String(id));
    const text = labelForMannequinId(id, mannequins, ext);
    let entry = sprites.get(String(id));
    if (!entry || entry.text !== text) {
      // 文字变化（角色顺序变）→ 重建纹理（缓存在 visual.__hyLabelTex）
    ...
    sprite.position.set(g.position.x, computeLabelPositionY(g.position.y, s, LABEL_OFFSET), g.position.z);
  });
...
export function syncSceneViewFocalLengthWith(bridge, sceneNode) {
  ...
  // 幂等用“上次请求的原始值”做标记，而非读 getter——setDefaultSceneFocalLength 会按相机约束钳制，
  // 若请求值超界，getter 回钳制后的值会与请求值恒不等 → setDefaultSceneFocalLength→requestRender 自循环。
  if (bridge.__hySceneFocalSet === focal) return;
  bridge.__hySceneFocalSet = focal;
