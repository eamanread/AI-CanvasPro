...
test("V3-D18c: 点1 偏航/俯仰 往返一致 (cameraAimFromYawPitchPatch ↔ deriveCameraInspector)", () => {
  const pos = { x: 2, y: 1.6, z: 5 };
  const scene = { cameras: [{ id: "c1", name: "机位1", position: pos, focalLength: 50 }] };
  const patch = cameraAimFromYawPitchPatch("c1", 30, 15, pos, 6);
  assert.ok(patch && patch.cameras.c1.lookAtPoint, "出注视点补丁");
  assert.equal(patch.cameras.c1.lookAtTarget, "manual", "改朝向 → 切手动注视");
  const ext = { cameras: { c1: { lookAtPoint: patch.cameras.c1.lookAtPoint } } };
  const insp = deriveCameraInspector(scene, ext, "c1");
  assert.equal(insp.yawDeg, 30, "偏航往返一致");
  assert.equal(insp.pitchDeg, 15, "俯仰往返一致");
  assert.ok(Math.abs(insp.lookDistance - 6) < 0.01, "注视距离保持(re-aim 不改距离)");
  // 俯仰夹 ±89° 防万向锁
  const p2 = cameraAimFromYawPitchPatch("c1", 0, 120, pos, 5);
  const i2 = deriveCameraInspector(scene, { cameras: { c1: { lookAtPoint: p2.cameras.c1.lookAtPoint } } }, "c1");
  assert.ok(i2.pitchDeg <= 89, "俯仰被夹到 ≤89");
  assert.equal(cameraAimFromYawPitchPatch(null, 0, 0, pos, 5), null, "缺 id 回 null");
});

test("V3-D18d: 点1 lookAtQuaternion 朝向往返 (forward=q·(0,0,-1)=归一(注视点-位置))", () => {
  const applyQuat = (q, v) => {
  ...
  const eye = { x: 1, y: 2, z: 3 }, target = { x: 4, y: 2, z: -1 };
  const q = lookAtQuaternion(eye, target);
  assert.ok(q, "出四元数");
  assert.ok(Math.abs(Math.hypot(q.x, q.y, q.z, q.w) - 1) < 1e-6, "单位四元数");
  const fwd = applyQuat(q, { x: 0, y: 0, z: -1 });
  const dx = target.x - eye.x, dy = target.y - eye.y, dz = target.z - eye.z, dl = Math.hypot(dx, dy, dz);
  assert.ok(Math.abs(fwd.x - dx / dl) < 1e-6 && Math.abs(fwd.y - dy / dl) < 1e-6 && Math.abs(fwd.z - dz / dl) < 1e-6, "相机 -Z 指向注视点");
  assert.ok(applyQuat(q, { x: 0, y: 1, z: 0 }).y > 0, "上方向不翻转(无 roll)");
  const q2 = lookAtQuaternion({ x: 0, y: 5, z: 0 }, { x: 0, y: 0, z: 0 });
  assert.ok(q2 && Number.isFinite(q2.w), "看正下方(退化)出有限四元数");
  assert.equal(lookAtQuaternion(null, target), null, "缺参回 null");
});
...
// ===== 立方体：恢复 + 场景树 + 命名（镜像人偶规则）=====
test("CUBE-1: cubeLabel/cubeDisplayName/cubeNamePatch 镜像人偶命名", () => {
  assert.equal(cubeLabel(0), "立方体1");
  assert.equal(cubeLabel(2), "立方体3");
  assert.equal(cubeDisplayName(null, "k1", 0), "立方体1", "无自定义名→派生标签");
  assert.equal(cubeDisplayName({ cubes: { k1: { name: "残墙" } } }, "k1", 0), "残墙", "自定义名优先");
  assert.deepEqual(cubeNamePatch("k1", "  桌子 "), { cubes: { k1: { name: "桌子" } } }, "trim");
  assert.deepEqual(cubeNamePatch("k1", "   "), { cubes: { k1: { name: "" } } }, "空串清回派生");
});

test("CUBE-2: deriveSceneTree 含 cubes 类别，标签/可见/锁定派生正确", () => {
  const scene = { cameras: [], mannequins: [], cubes: [{ id: "k1", colorKey: "blue" }, { id: "k2", colorKey: "red" }] };
  const ext = { cubes: { k1: { name: "残墙" } }, visibility: { cubeIds: ["k2"] }, locked: { cubeIds: ["k1"] } };
  const tree = deriveSceneTree(scene, ext);
  assert.equal(tree.cubes.length, 2);
  assert.equal(tree.cubes[0].label, "残墙");
  assert.equal(tree.cubes[1].label, "立方体2", "无名→立方体N");
  assert.equal(tree.cubes[0].type, "cube");
  assert.equal(tree.cubes[0].locked, true, "k1 锁定");
  assert.equal(tree.cubes[1].visible, false, "k2 隐藏");
});

test("CUBE-3: cube 可见性/锁定走独立 cubeIds 桶，不与人偶 mannequinIds 串", () => {
  const ext = { visibility: { mannequinIds: ["m1"], cubeIds: ["k1"] } };
  assert.equal(isObjectHidden("cube", "k1", ext), true);
  assert.equal(isObjectHidden("cube", "m1", ext), false, "人偶 id 不命中 cube 桶");
  assert.equal(isObjectHidden("mannequin", "k1", ext), false, "cube id 不命中人偶桶");
  // toggle 立方体可见性 → 只动 cubeIds
  assert.deepEqual(toggleVisibilityPatch("cube", "k2", ext), { visibility: { cubeIds: ["k1", "k2"] } });
});

test("CUBE-4: deriveInspector 选中 cube → 名称+变换（镜像人偶）", () => {
  const scene = {
    cubes: [{ id: "k1", colorKey: "blue", position: { x: 1, y: 1.5, z: 0 }, rotation: { x: 0, y: 0, z: 0 }, scale: 3 }],
    mannequins: [],
    selection: { selectedObjectType: "cube", selectedObjectId: "k1" },
  };
  const insp = deriveInspector(scene, { cubes: { k1: { name: "残墙" } } });
  assert.equal(insp.kind, "cube");
  assert.equal(insp.title, "立方体");
  assert.equal(insp.label, "残墙");
  assert.equal(insp.editable, true);
  assert.equal(insp.fields.posY, 1.5);
  assert.equal(insp.fields.scale, 3);
});

test("CUBE-5: labelForCubeId 自定义名优先（头顶名牌跟随改名）+ filterSceneTree 过滤 cubes", () => {
  const cubes = [{ id: "k1" }, { id: "k2" }];
  assert.equal(labelForCubeId("k2", cubes, { cubes: { k2: { name: "掩体" } } }), "掩体");
  assert.equal(labelForCubeId("k1", cubes, null), "立方体1");
  const tree = deriveSceneTree({ cameras: [], mannequins: [], cubes }, { cubes: { k2: { name: "掩体" } } });
  assert.equal(filterSceneTree(tree, "掩体").cubes.length, 1);
  assert.equal(filterSceneTree(tree, "掩体").cubes[0].id, "k2");
});
