...
function mannequinDisplayName(directorExt, id, idx) {
...
// 立方体命名（完全镜像人偶规则）：自定义名优先（directorExt.cubes[id].name），否则回落 立方体N。
export function cubeLabel(idx) {
  const i = Number.isFinite(idx) && idx >= 0 ? idx : 0;
  return `立方体${i + 1}`;
}
export function cubeDisplayName(directorExt, id, idx) {
  const ext = directorExt && directorExt.cubes && directorExt.cubes[String(id)];
  const name = ext && typeof ext.name === "string" ? ext.name.trim() : "";
  return name || cubeLabel(idx);
}
export function cubeNamePatch(id, name) {
  const v = typeof name === "string" ? name.trim() : "";
  return { cubes: { [String(id)]: { name: v } } };
}
// 名牌文字（立方体）：自定义名优先，否则按下标 立方体N。
export function labelForCubeId(id, cubes, directorExt) {
  const list = Array.isArray(cubes) ? cubes : [];
  const idx = list.findIndex((c) => String(c?.id) === String(id));
  return cubeDisplayName(directorExt, id, idx >= 0 ? idx : 0);
}
...
// 可见性/锁定按对象类别分桶:camera→cameraIds、cube→cubeIds、其余(人偶)→mannequinIds。
function idKey(kind) {
  return kind === "camera" ? "cameraIds" : (kind === "cube" ? "cubeIds" : "mannequinIds");
}
function extIdArray(ext, group, kind) {
  const g = ext && ext[group];
  const arr = g && g[idKey(kind)];
...
export function deriveSceneTree(sceneState, directorExt) {
  ...
  // 立方体/道具（可命名,与人偶同规则；可见性/锁定走 directorExt.*.cubeIds）
  const cubes = (Array.isArray(sceneState?.cubes) ? sceneState.cubes : []).map((c, i) => {
    const id = String(c?.id ?? "");
    return {
      id,
      type: "cube",
      label: cubeDisplayName(directorExt, id, i),
      colorHex: mannequinColorHex(c?.colorKey),
      visible: !isObjectHidden("cube", id, directorExt),
      locked: isObjectLocked("cube", id, directorExt),
    };
  });
  return { cameras, characters, cubes };
}
...
export function filterSceneTree(tree, query) {
  ...
  return {
    cameras: tree.cameras.filter(hit),
    characters: tree.characters.filter(hit),
    cubes: (tree.cubes || []).filter(hit),
  };
}
...
export function deriveInspector(sceneState, directorExt) {
  ...
  // 立方体（可经原生 selection 选中）：名称(directorExt.cubes[id].name) + 位置/旋转/缩放 + 颜色,镜像人偶。
  if (sel.selectedObjectType === "cube") {
    const list = Array.isArray(sceneState?.cubes) ? sceneState.cubes : [];
    const idx = list.findIndex((c) => String(c?.id) === String(sel.selectedObjectId));
    if (idx < 0) return null;
    const c = list[idx];
    const multi = Array.isArray(sel.selectedObjects) && sel.selectedObjects.length > 1;
    const locked = isObjectLocked("cube", c.id, directorExt);
    return {
      kind: "cube",
      id: String(c.id),
      title: "立方体",
      label: cubeDisplayName(directorExt, c.id, idx),
      colorHex: mannequinColorHex(c.colorKey),
      colorKey: String(c.colorKey || "").toLowerCase(),
      locked,
      editable: !multi && !locked,
      fields: {
        posX: round2(c.position?.x),
        posY: round2(c.position?.y),
        posZ: round2(c.position?.z),
        rotXDeg: radToDeg(c.rotation?.x),
        rotYDeg: radToDeg(c.rotation?.y),
        rotZDeg: radToDeg(c.rotation?.z),
        scale: round2(Number.isFinite(c.scale) && c.scale > 0 ? c.scale : 1),
      },
    };
  }
...
export function deriveCameraInspector(sceneState, directorExt, cameraId) {
  ...
  // 点1:由 位置→注视点 推「偏航/俯仰」(检查器可调,re-aim 相机)。yaw0/pitch0 朝 -Z;yaw 右转为正、pitch 上仰为正。
  const look = ext.lookAtPoint && Number.isFinite(ext.lookAtPoint.x) ? ext.lookAtPoint : { x: 0, y: 1.2, z: 0 };
  const dvx = look.x - pos.x, dvy = look.y - pos.y, dvz = look.z - pos.z;
  const lookDistance = Math.hypot(dvx, dvy, dvz) || 1;
  const yawDeg = Math.round((Math.atan2(dvx, -dvz) * 180) / Math.PI);
  const pitchDeg = Math.round((Math.asin(Math.max(-1, Math.min(1, dvy / lookDistance))) * 180) / Math.PI);
  return {
    kind: "camera",
    id: String(c.id),
    title: "摄像机",
    ...
    yawDeg,
    pitchDeg,
    lookDistance: round2(lookDistance),
...
// 点1:相机「偏航/俯仰」→ 注视点(re-aim,相机位置不动)。与 deriveCameraInspector 的 yaw/pitch 推导互逆。
// yaw 右转为正、pitch 上仰为正;按当前 注视距离 把注视点放到新朝向上。pitch 夹 ±89° 防万向锁。
export function cameraAimFromYawPitchPatch(cameraId, yawDeg, pitchDeg, position, distance) {
  if (!cameraId || !position) return null;
  const yaw = ((Number(yawDeg) || 0) * Math.PI) / 180;
  const pitch = (Math.max(-89, Math.min(89, Number(pitchDeg) || 0)) * Math.PI) / 180;
  const d = Number(distance) > 0.01 ? Number(distance) : 5;
  const dir = { x: Math.sin(yaw) * Math.cos(pitch), y: Math.sin(pitch), z: -Math.cos(yaw) * Math.cos(pitch) };
  return {
    cameras: {
      [String(cameraId)]: {
        lookAtTarget: "manual",
        lookAtPoint: { x: position.x + dir.x * d, y: position.y + dir.y * d, z: position.z + dir.z * d },
      },
    },
  };
}

// 由 相机位置→注视点 求「看向」四元数（相机 -Z 朝注视点、+Y 上）。
// 用途:把检查器编辑后的机位朝向同步进 sceneNode.cameras[id].quaternion,使桥 _syncCameras 重转「相机标记」
// （标记的 quaternion 直接取自 sceneNode 位姿,不读 directorExt;桥相机前向为 -Z,见 forward=q·(0,0,-1)）。
// 退化(注视点落在位置正上/正下,up 与视线平行)时换备用 up=(0,0,1) 防叉积归零。返回单位四元数 {x,y,z,w}。
export function lookAtQuaternion(eye, target, up) {
  if (!eye || !target || !Number.isFinite(eye.x) || !Number.isFinite(target.x)) return null;
  const U = up && Number.isFinite(up.x) ? up : { x: 0, y: 1, z: 0 };
  // zAxis = 归一(eye - target):相机 +Z 背向注视点
  let zx = eye.x - target.x, zy = eye.y - target.y, zz = eye.z - target.z;
  let zl = Math.hypot(zx, zy, zz);
  if (zl < 1e-6) { zx = 0; zy = 0; zz = 1; zl = 1; } // 位置=注视点 → 取默认朝向(看向 -Z)
  zx /= zl; zy /= zl; zz /= zl;
  // xAxis = 归一(up × z)
  let xx = U.y * zz - U.z * zy, xy = U.z * zx - U.x * zz, xz = U.x * zy - U.y * zx;
  let xl = Math.hypot(xx, xy, xz);
  if (xl < 1e-6) { // up 与视线平行(看正上/正下) → 换备用 up=(0,0,1)
    xx = 0 * zz - 1 * zy; xy = 1 * zx - 0 * zz; xz = 0 * zy - 0 * zx; // (0,0,1) × z
    xl = Math.hypot(xx, xy, xz) || 1;
  }
  xx /= xl; xy /= xl; xz /= xl;
  // yAxis = z × x
  const yx = zy * xz - zz * xy, yy = zz * xx - zx * xz, yz = zx * xy - zy * xx;
  // 旋转矩阵(列=各基向量) → 四元数(同 Three.js Quaternion.setFromRotationMatrix)
  const m11 = xx, m12 = yx, m13 = zx;
  const m21 = xy, m22 = yy, m23 = zy;
...
export function deriveSceneGlobal(sceneState, directorExt) {
  ...
  return {
    ...
    skyColor,
    environmentMode: sceneState?.environmentMode === "night" ? "night" : "day", // 点8:白天/黑夜(默认 day)
